...
public class S3Library {
     ...
     */
    public String getUploadedFileName(String fileUrl) {
        try {
            String tmpFileUrl = fileUrl.replace(cfImgURL, "");// 도메인 제거
            int index = tmpFileUrl.lastIndexOf("/");

            return tmpFileUrl.substring(index + 1); // '/' 이후의 파일 이름만 추출
        } catch (StringIndexOutOfBoundsException e) {
    ...
    public void checkUploadFiles(List<MultipartFile> uploadFiles) {
        ...
        for (MultipartFile multipartFile : uploadFiles) {
            ...
            if (ObjectUtils.isEmpty(contentType) || !contentType.equals("image")) {
                throw new CustomException(CustomError.CONTENTS_REGISTER_IMAGE_EXTENSION_ERROR); // 허용하지 않는 확장자를 가진 파일입니다.
            }
            ...
            if (size > UPLOAD_MAX_SIZE) {
                throw new CustomException(CustomError.CONTENTS_REGISTER_IMAGE_SIZE_ERROR);     // 등록할 이미지 용량이 너무 큽니다.
            }
